...
- (void)createFilterFBOofSize:(CGSize)currentFBOSize;
{
    runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([GPUImageContext supportsFastTextureUpload] && preparedToCaptureImage)
        {
            ...
            if (err)
            {
                DLog(@"FBO size: %f, %f", currentFBOSize.width, currentFBOSize.height);
...
//            glDrawBuffers(1, &att);
        ...
        //    DLog(@"Filter size: %f, %f for filter: %@", currentFBOSize.width, currentFBOSize.height, self);
...
{
//    DLog(@"Cleaning up output filter image: %@", self);
    [self destroyFilterFBO];
    [self deleteOutputTexture];
}
