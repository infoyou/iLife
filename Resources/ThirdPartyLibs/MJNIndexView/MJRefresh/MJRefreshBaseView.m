...
//    CGFloat validY; //合理的Y值
//    int viewType; //view的类型
}
...
//    self.backgroundColor = [UIColor colorWithRed:color green:color blue:color alpha:1];
    ...
    // 3.状态标签
    [self addSubview:_statusLabel = [self labelWithFontSize:11]];
    
    // 4.箭头图片
    UIImageView *arrowImage = [[UIImageView alloc] init];
//    arrowImage.contentMode = UIViewContentModeScaleAspectFit;
//    arrowImage.image = [UIImage imageNamed:kSrcName(@"arrow.png")];
    arrowImage.image = [UIImage imageNamed:@"MJRefresh.bundle/arrow.png"];
...
#ifdef NeedAudio
    // 7.加载音频
    _pullId = [self loadId:@"MJRefresh.bundle/pull.wav"];
    _normalId = [self loadId:@"MJRefresh.bundle/normal.wav"];
    _refreshingId = [self loadId:@"MJRefresh.bundle/refreshing.wav"];
    _endRefreshId = [self loadId:@"MJRefresh.bundle/end_refreshing.wav"];
#endif
}
...
- (void)setFrame:(CGRect)frame
{
    ...
    // 3.箭头
    CGFloat arrowX = 20;
    _arrowImage.frame = CGRectMake(arrowX, statusY, 12.0f, 26.0f);
    
    // 4.指示器
    _activityView.bounds = CGRectMake(0, 0, 15.0f, 15.0f);
