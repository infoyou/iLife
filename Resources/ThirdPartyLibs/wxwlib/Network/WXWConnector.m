...
- (void)connectionStopHandler {
    ...
    if (needCookie) {
        ...
        for (NSHTTPCookie *cookie in [cookieJar cookies]) {
            ...
            [AppManager instance].pgCookie = [cookie valueForKey:NSHTTPCookieValue];
            DLog(@"pgCookie = %@", [AppManager instance].pgCookie);
        }
    }
...
- (NSData *)assembleContentData:(NSDictionary *)dic {
    ...
    //DLog(@"parame: %@", [[[NSString alloc] initWithData:contentData encoding:NSUTF8StringEncoding] autorelease]);
