...
+ (void)showNotificationOnTopWithMsg:(NSString *)msg
    ...
    if (nil == infoGradientBackgroundView) {
        ...
        infoGradientBackgroundView.layer.shadowColor = [UIColor blackColor].CGColor;
        infoGradientBackgroundView.layer.shadowOpacity = 0.2f;
        infoGradientBackgroundView.layer.shadowOffset = CGSizeMake(0, 0.0f);
//        infoGradientBackgroundView.layer.shadowOffset = CGSizeMake(10, 10);//阴影距离
...
+ (BOOL)shouldLoadOlderItems:(UIScrollView *)scrollView
    ...
    if (scrollView.isDragging) {
        ...
        if ((footerView.state == PULL_FOOTER_NORMAL) &&
            ...
            // means scroll down, if user scrolls up, no need to load old post
            !reloading) {
