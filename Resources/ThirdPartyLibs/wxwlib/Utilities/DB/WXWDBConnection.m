...
+ (sqlite3 *)prepareBizDB {
     ...
     */
	imgStmt = [self statementWithQuery:"create table if not exists userTable (userId TEXT PRIMARY KEY, userName TEXT, userNameEn TEXT, chatId TEXT, customerId TEXT, userImageUrl TEXT, userTel TEXT, userEmail TEXT, userStatus TEXT, userCode TEXT, groupId TEXT, userDept TEXT, userTitle TEXT, userRole TEXT, isFriend int, isDelete int, userGender int, userCellphone TEXT, userBirthDay TEXT, superName TEXT);"];
    
    ret = [imgStmt step];
	if (ret != SQLITE_DONE) {
		debugLog(@"Failed to create 'userTable' table.");
		NSAssert1(0, @"Failed to create 'userTable' table.", nil);
	}
    //------------------------------------------------------------
...
+ (void)rollback {
    char *errMsg;
	sqlite3_exec(theDB, "ROLLBACK", NULL, NULL, &errMsg);
}
